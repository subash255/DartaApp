...
@section('content')
    ...
    <div class="p-4 bg-white shadow-lg -mt-12 mx-4 z-20 rounded-lg">
        ...
        <div class="container mx-auto p-6">
            <form method="POST" action="{{ route('admin.company.step3.update', $company->id) }}"
                ...
                <div class="mb-6 flex items-center justify-start">
                    <input type="hidden" id="copystep2data_hidden" name="copystep2data"
                        value="{{ old('copystep2data', $company->copystep2data ?? 0) }}">
                    ...
                    <label for="copyStep2Data" class="ml-2 text-sm font-medium text-gray-900">Same address as
                        citizenship?</label>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="mb-6">
                        <label for="houseownername" class="block mb-2 text-sm font-medium text-gray-900">House Owner
                            Name</label>
                        <input type="text" name="houseownername" id="houseownername"
                            value="{{ old('houseownername', $company->houseownername ?? '') }}"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block w-full p-2.5"
                            required>
                    </div>
                    <div class="mb-6">
                        <label for="houseownertole" class="block mb-2 text-sm font-medium text-gray-900">Tole</label>
                        <input type="text" name="hotole" id="hotole"
                            value="{{ old('hotole', $company->hotole ?? '') }}"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block w-full p-2.5"
                            required>
                    </div>
                    <div class="mb-6">
                        <label for="houseownermunicipality"
                    ...
                    <div class="mb-6">
                        ...
                        <input type="text" name="howard" id="howard"
                            value="{{ old('howard', $company->howard ?? '') }}"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-orange-500 focus:border-orange-500 block w-full p-2.5"
                            required>
                    </div>
                    <div class="mb-6">
                        <label for="houseownerdistrict"
                    ...
                    <div class="mb-6">
                        ...
                        @if (!empty($company->holalpurja))
                            <img src="{{ asset('document/' . $company->holalpurja) }}" alt="Lalpurja Image"
                                class="mt-2 max-w-xs h-auto rounded-lg">
                        @endif
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        @if (!empty($company->hotiro))
                            <img src="{{ asset('document/' . $company->hotiro) }}" alt="Tiro Image"
                                class="mt-2 max-w-xs h-auto rounded-lg">
                        @endif
                    </div>
                </div>
                <h1 class="text-xl  font-semibold">Location</h1>
                <p class="text-sm text-gray-500">Drag the marker to set the exact location of the company.</p>
