...
@section('content')
                        ...
                        <tr class="bg-white hover:bg-gray-50">
                            ...
                            <td class="border border-gray-300 px-4 py-2">
                                <label for="status{{ $customer->id }}" class="inline-flex items-center cursor-pointer">
                                    <input id="status{{ $customer->id }}" type="checkbox" class="hidden toggle-switch" data-id="{{ $customer->id }}" {{ $customer->status ? 'checked' : '' }} />
    ...
    <script>
       ...
       toggle.addEventListener('change', function() {
           const customerId = this.getAttribute('data-id'); // Get the category ID from the data-id attribute
