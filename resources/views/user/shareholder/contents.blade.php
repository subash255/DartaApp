<div class="mb-8">
    <div class="flex justify-between mb-2">
        ...
        <a href="{{ route('user.shareholder.step2',  $userDetail->id ?? '') }}"class="text-xs font-bold inline-block py-1 px-2 rounded-full text-gray-800 bg-orange-200 opacity-50"
            id="step2">Address per Citizenship</a>
        <a href="{{ route('user.shareholder.step3',  $userDetail->id ?? '') }}"class="text-xs font-bold inline-block py-1 px-2 rounded-full text-gray-800 bg-orange-200 opacity-50"
            id="step3">Current Address</a>
        <a href="{{ route('user.shareholder.step4',  $userDetail->id ?? '') }}"class="text-xs font-bold inline-block py-1 px-2 rounded-full text-gray-800 bg-orange-200 opacity-50"
            id="step4">Add. Info</a>
        <a href="{{ route('user.shareholder.step5',  $userDetail->id ?? '') }}"class="text-xs font-bold inline-block py-1 px-2 rounded-full text-gray-800 bg-orange-200 opacity-50"
            id="step5">Share Details</a>
        <a href="{{ route('user.shareholder.step6',  $userDetail->id ?? '') }}"class="text-xs font-bold inline-block py-1 px-2 rounded-full text-gray-800 bg-orange-200 opacity-50"
            id="step6">Acc. Details</a>
    </div>
    <div class="overflow-hidden h-2 mb-4 text-xs flex rounded bg-orange-200">
        <div id="progress-bar"
            class="shadow-none flex flex-col text-center whitespace-nowrap text-white justify-center bg-orange-500 w-0 transition-all duration-500 ease-in-out">
        </div>
    </div>
</div>
